...
function getBannerFromSearch(search: string): { type: 'error' | 'success'; message: string } | null {
  const params = new URLSearchParams(search);
  const authError = params.get('error');
  const success = params.get('success');

  if (authError === 'session_invalid')
    return { type: 'error', message: 'Session expired — please sign in again.' };
  if (authError === 'session_refresh_failed')
    return { type: 'error', message: 'MFA setup complete, but session could not be refreshed. Please sign in again.' };
  if (authError === 'auth_callback_failed')
    return { type: 'error', message: 'The password reset link is invalid or has expired. Please request a new one.' };
  if (success === 'password_reset')
    return { type: 'success', message: 'Password updated. You can now sign in with your new password.' };
...
export default function SignInPage() {
  ...
  const [error, setError] = useState<string | null>(() => {
    ...
    const banner = getBannerFromSearch(window.location.search);
    return banner?.type === 'error' ? banner.message : null;
  });
  const [success, setSuccess] = useState<string | null>(() => {
    if (typeof window === 'undefined') return null;
    const banner = getBannerFromSearch(window.location.search);
    return banner?.type === 'success' ? banner.message : null;
  });
    ...
    try {
      ...
      if (claims.user_role && MFA_REQUIRED_ROLES.includes(claims.user_role)) {
        ...
        if (!aalError && aalData?.currentLevel !== 'aal2' && aalData?.nextLevel === 'aal2') {
          ...
          const factor = factorsError ? null : (factorsData?.totp.find((f) => f.status === 'verified') ?? null);
      ...
      // All checks passed — navigate to dashboard (or original destination)
      const dest = getSafeRedirect(window.location.search);
      router.push(dest);
      router.refresh();
